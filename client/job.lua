		...
		if data.current.value == 'citizen_interaction' then
			ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'citizen_interaction', {
				...
				if data.current.value == 'search' then
					TriggerServerEvent('esx_firejob:svsearch')
	...
	ESX.TriggerServerCallback('esx_firejob:getItemAmount', function(quantity)
		if quantity > 0 then
			...
			if IsPedDeadOrDying(closestPlayerPed, 1) then
				...
				TriggerServerEvent('esx_firejob:removeItem', 'medikit')
				TriggerServerEvent('esx_firejob:revive', GetPlayerServerId(closestPlayer))
			else
...
CreateThread(function()
	while true do
		...
		if ESX.GetPlayerData().job and ESX.GetPlayerData().job.name == 'fire' then
			...
			if isInMarker and not HasAlreadyEnteredMarker or (isInMarker and (LastHospital ~= currentHospital or LastPart ~= currentPart or LastPartNum ~= currentPartNum)) then
				...
				then
					TriggerEvent('esx_firejob:hasExitedMarker', LastHospital, LastPart, LastPartNum)
					hasExited = true
				end

				HasAlreadyEnteredMarker, LastHospital, LastPart, LastPartNum = true, currentHospital, currentPart, currentPartNum

				TriggerEvent('esx_firejob:hasEnteredMarker', currentHospital, currentPart, currentPartNum)
			end
...
AddEventHandler('esx_firejob:hasEnteredMarker', function(hospital, part, partNum)
...
AddEventHandler('esx_firejob:hasExitedMarker', function(hospital, part, partNum)
...
RegisterCommand("fire", function(src)
	if ESX.GetPlayerData().job and ESX.GetPlayerData().job.name == 'fire' and not ESX.GetPlayerData().dead then 
		OpenMobileAmbulanceActionsMenu()
	end
end)

RegisterKeyMapping("fire", "Open Fire Actions Menu", "keyboard", "k")
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'cloakroom', {
		...
		elseif data.current.value == 'ambulance_wear' then
			ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin, jobSkin)
				...
				isOnDuty = true
				TriggerEvent('esx_firejob:setDeadPlayers', deadPlayers)
			end)
		end
...
function OpenPharmacyMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'pharmacy', {
		...
		TriggerServerEvent('esx_firejob:giveItem', data.current.item, data.current.value)
	end, function(data, menu)
		menu.close()
	end)
end
...
RegisterNetEvent('esx_firejob:setDeadPlayers')
AddEventHandler('esx_firejob:setDeadPlayers', function(_deadPlayers)
